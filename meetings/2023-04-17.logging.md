...
At the generic service level, in addition to hostname (machine) and FQSN (microservice), it would be interesting to capture the following:

- deprecated (boolean) - a mechanism to identify the use of any deprecated functioality in the 
...
- duration
- status-code for the request (where applicable)
...
- UI
  - Every request/api call is a key event
    ...
    - Byte count is not that significant due to the use of presigned retrievals 
    - Count logins
